...
class HomeV extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _HomeVS();

}

class _HomeVS extends State<HomeV> {
  ...
  void initState() {
    super.initState();
    _initializeAssistantVM();
  }

  Future<void> _initializeAssistantVM() async {
    final token = await AuthApi.getAccessToken();
    if (token == null) return;
    final profile = await ProfileAPI.getProfile(token);
    if (profile == null) return;
    _assistantVM.name = profile.name;
    _assistantVM.email = profile.email;
    _assistantVM.phoneNumber = profile.phoneNumber;
    _assistantVM.currentLat = profile.currentLat;
    _assistantVM.currentLng = profile.currentLng;
    _assistantVM.serviceType = profile.serviceType;
    _assistantVM.vehicleType = profile.vehicleType;
  }
